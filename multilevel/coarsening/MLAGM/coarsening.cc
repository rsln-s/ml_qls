...
#if dbl_CO_calcP >= 3
    cout <<funcIdx+"{" << this->cc_name <<"} Average Future Volume:"<<
           sum_future_volume / vertexs.getSize() << endl;
...
//        ETimer t_sseed;
    ...
    //NOTE: Always notice that set avg-future-volume in advance
    num_strong_seeds = vertexs.selectSeeds();
...
//================ Add points from F to C =======================
    ...
    //go through all F_nodes
    for (auto it = F_nodes_.begin(); it != F_nodes_.end(); ++it) {
        sigma_C_ = 0 ;
        sigma_V_ = 0;

        //ncols : number if non-zeros in the row
        MatGetRow(WA, it->node_index ,&ncols,&cols,&vals);
#if dbl_CO_calcP >=9
        printf("CHECK: is it eligible to add from F to C node_index:%lu ncols:%d\n",
               it->node_index, ncols );
#endif
        for (j=0; j<ncols; j++) {
...
// columns : coarse points   => col_dimension : number of the coarse points => num_col
    ...
    // try to reserve space for only number of final non zero entries for each fine node (e.g. 4)
    MatCreateSeqAIJ(PETSC_COMM_SELF,num_row,this->num_coarse_points,
                    paramsInst->get_coarse_r() ,PETSC_NULL, &P);
...
#endif
    //All nodes in V (i == node id )
    for(i =0; i <num_row; ++i){
        if(debug) cout << i << ", ";

        //if the node is seed ==> value == 1
        if(vertexs.getNode(i).getIsSeed()){
            ...
            for (j=0; j<ncols; j++) {
                //if J belongs to N_i
                if(vertexs.getNode(cols[j]).getIsSeed()){
                    // add the node_index and the value to a vector
                    // changed at 071616-1710 (this one is based on 2-sum paper)
                    //equation 2 at page 242 min 2-sum paper Dr. Safro
                    filter_nodes_p.push_back(tmp_filter_p(seeds_indices[cols[j]],
                                             vals[j]/sigma_w_ik) );
                }
            }
...
//               level, name.c_str(), num_row, num_col, cnt_nnz);
        printf("[CO][CSN] level:%d, num_row:%d, num_col:%d, num_edges(nnz/2)=%g\n",
               level, num_row, num_col, cnt_nnz/2);
