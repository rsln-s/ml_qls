...
void Config_params::print_amg_coarse_quantum_params(){
    cout << "\nds_path: "              << get_ds_path()           <<
            "\nds_name: "              << get_ds_name()           <<
            ...
            "\ncs_max_coarse_level: "  << get_cs_max_coarse_level()  <<
            "\ncs_use_real_points: "   << get_cs_use_real_points()   <<
            "\ncs_weak_edges_ft: "     << get_cs_weak_edges_ft()     << endl;
}
...
//            set_inputs_file_names();
    ...
    case convert_files:
        read_convert_files_parameters(root, argc, argv);
        print_convert_files_params();
        break;

    case amg_coarse_quantum:
        read_amg_coarse_quantum_parameters(root, argc, argv);
        print_amg_coarse_quantum_params();
        break;
    }
}
...
void Config_params::read_amg_coarse_quantum_parameters(pugi::xml_node& root,
                                                    int argc, char * argv[]){
    ...
    cs_weak_edges_ft    = root.child("cs_weak_edges_ft").attribute("doubleVal").as_double();
    /// read the parameters from input arguments ()
    parser_.add_option("--ds_p") .dest("ds_path")
            .set_default(ds_path);
    parser_.add_option("-f", "--ds_f", "--file") .dest("ds_name")
            .set_default(ds_name);
    parser_.add_option("--cs_eta") .dest("coarse_Eta")
            .set_default(coarse_Eta);
    parser_.add_option("-t", "--cs_t" ) .dest("coarse_threshold")
            .set_default(coarse_threshold);
    parser_.add_option("-q", "--cs_q" ) .dest("coarse_q")
            .set_default(coarse_q);
    parser_.add_option("-r", "--cs_r" ) .dest("coarse_r")
            .set_default(coarse_r);
    parser_.add_option("--cs_m") .dest("cs_max_coarse_level")
            .set_default(cs_max_coarse_level);
    parser_.add_option("--cs_we") .dest("cs_weak_edges_ft")
            .set_default(cs_weak_edges_ft);

    this->options_ = parser_.parse_args(argc, argv);
    std::vector<std::string> args = parser_.args();
    cout << "[CP] input amg_coarse_quantum parameters are read" << endl;
}
