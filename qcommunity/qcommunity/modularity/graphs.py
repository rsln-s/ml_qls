...
from networkx.generators.classic import barbell_graph, ladder_graph
...
def get_ladder_graph(left, right, **kwargs):
    # kwargs are ignored
    if left != right:
        raise ValueError(
            "Left ({}) != right ({}), ignoring right ({})".format(left, right, right))
    size = left
    logging.info('Generating {} node ladder graph'.format(size))
    G = ladder_graph(size)
    return G, None
...
def get_erdos_renyi_graph(left, right, p=0.5, seed=42, **kwargs):
    ...
    if p is None:
        warnings.warn("Received p=None, defaulting to p=0.5")
        p = 0.5
...
if __name__ == "__main__":
    ...
    G, solution_bitstring = get_connected_caveman_graph(2,6)
    print("Modularity: ",compute_modularity_c(G, solution_bitstring), " normalized: ", compute_modularity_c(G, solution_bitstring) / (4 * G.number_of_edges()))
